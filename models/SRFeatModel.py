    ...
    def optimize_parameters(self, step):
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            if self.cri_fea:  # feature loss
                ...
                l_g_total += l_g_fea
            # G gan + cls loss
            pred_g_fakeI = self.netD1(self.fake_H)

            l_g_ganI = self.l_gan_w * (self.cri_gan(pred_g_fakeI, True))
            l_g_total += l_g_ganI

            real_fea = self.netF(self.var_H).detach()
            fake_fea = self.netF(self.fake_H)
            #print("size of vgg feat: {},{},{},{}".format(np.size(fake_fea,0),np.size(fake_fea,1),np.size(fake_fea,2),np.size(fake_fea,3)))
            pred_g_fakeF = self.netD2(fake_fea)

            l_g_ganF = self.l_gan_w * (self.cri_gan(pred_g_fakeF, True))
        ...
        l_d_total = 0
        pred_d_real = self.netD1(self.var_ref)
        pred_d_fake = self.netD1(self.fake_H.detach())  # detach to avoid BP to G
        ...
        l_d_total = 0
        real_fea = self.netF(self.var_H).detach()
        fake_fea = self.netF(self.fake_H.detach()) # detach to avoid BP to G
        pred_d_real = self.netD2(real_fea)
        pred_d_fake = self.netD2(fake_fea)  
        l_d_real = self.cri_gan(pred_d_real, True)
        l_d_fake = self.cri_gan(pred_d_fake, False)
        ...
        if step % self.D_update_ratio == 0 and step > self.D_init_iters:
            ...
            self.log_dict['l_g_gan'] = l_g_ganI.item()
        # D
